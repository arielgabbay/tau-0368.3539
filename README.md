...
```
python3.8 ctf_scripts/prepare.py <directory_name> -n <num_groups>
```
...
```
base_directory/
	group_XX/ - the directory for group number XX
		stage_XX/ - the directory for stage XX of the CTF
			flag.bin - a file containing the flag for this stage
...
To modify the number of stages or what's created for each stage, modify the `prepare.py` script.
...
We can then run the server (a different server needs to be run for every group and stage; TODO: automate this)

```
mbedtls/programs/ssl/ssl_server3 key_file=<privkey_file> crt_file=<cert_file> force_version=tls12 force_ciphersuite=TLS-RSA-PSK-WITH-AES-128-CBC-SHA256 psk=abcdef
```

To use PKCS 1 v1.5 (for a Bleichenbacher oracle), this line is sufficient. To use PKCS 1 OAEP (for a Manger oracle), add the argument `oaep_padding=1`. The private key and certificate files are the relevant files (`priv.key.pem` and `cert.crt`, respectively) in the server's files generated by `prepare.py` and shown above.
